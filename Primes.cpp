...
template <unsigned int... valueList, unsigned int begin,
          unsigned int possibleFactor>
struct Move<Ints<valueList...>, begin, possibleFactor, true>
    : FactorsHelper<Ints<valueList..., possibleFactor>, possibleFactor - 1,
                    begin> {};
...
struct FactorsHelper<Ints<factorList...>, 1, begin> {
    ...
    static constexpr std::array<unsigned int, sizeof...(factorList)> values = {
        {factorList...}};
};

// Performs the bulk of the Factor work. Moves the current value to the
// factorList if the start value is divisible by it (i.e. if it is a factor)
template <unsigned int... factorList, unsigned int possibleFactor,
          unsigned int begin>
struct FactorsHelper<Ints<factorList...>, possibleFactor, begin>
    : Move<Ints<factorList...>, begin, possibleFactor,
           (begin % possibleFactor == 0)> {};
...
struct Remove<Ints<head, intList...>, false>
    : public PrimeHelper<Ints<head - 1, intList...>> {};

// The head of the list is prime, so keep it and check the next value
template <unsigned int... intList, unsigned int head>
struct Remove<Ints<head, intList...>, true>
    : public PrimeHelper<Ints<head - 1, head, intList...>> {};
...
struct PrimeHelper<Ints<2, intList...>> {
    ...
    static constexpr std::array<unsigned int, sizeof...(intList)+1> values = {
        {2, intList...}};
};

// Bulk of the prime work. Remove the head from the prime list if its factors
// are non-empty
template <unsigned int... intList, unsigned int head>
struct PrimeHelper<Ints<head, intList...>>
    : public Remove<Ints<head, intList...>,
                    std::is_same<typename Factors<head>::value,
                                 Ints<>>::value> {};
...
int main() {
    ...
    static_assert(primes[0] == 2 && primes[1] == 3 && primes[2] == 5 &&
                      primes[3] == 7 && primes[4] == 11 && primes[5] == 13 &&
                      primes[6] == 17 && primes[7] == 19,
                  "Incorrect value of primes");
}
