...
constexpr int lesser(int left, int right) {
    return (left < right) ? left : right;
}

// Find the least element in a list of ints
template <int... T>
struct Least;

// The least of a single element list is the element
template <int x>
struct Least<x> {
    enum { value = x };
};

// The least value in a set is the Least of the lesser of the first two and the
// rest of the set
template <int x1, int x2, int... xs>
struct Least<x1, x2, xs...> : public Least<lesser(x1, x2), xs...> {};
...
struct Remove<Ints<ints...>, target>
    : Check<Ints<ints...>, Ints<>, target, false> {};
...
// Base case of check. Stop checking for the target when the unchecked set is
// empty
...
struct Check<Ints<head, unchecked...>, Ints<checked...>, target, found>
    : public Move<Ints<head, unchecked...>, Ints<checked...>, target,
                  (head == target)> {};

template <typename unsorted, typename sorted = Ints<>>
struct SortHelper;
...
template <int... ints>
const std::array<int, sizeof...(ints)>
...
struct SortHelper<Ints<head, unsortedInts...>, Ints<sortedInts...>>
    : public SortHelper<typename Remove<Ints<head, unsortedInts...>,
                                        Least<head,
                                              unsortedInts...>::value>::value,
                        Ints<sortedInts...,
                             Least<head, unsortedInts...>::value>> {};
